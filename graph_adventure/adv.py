...
while q.size() > 0:
    ...
    if v.id not in visited:
        ...
        for direction in v.getExits():
            d[direction] = '?'
        visited[v.id] = d
    #randomly pick a direction that has not been visited 
    for direction in v.getExits():
        if visited[v.id][direction] == '?':
            # mark it as visited
            if direction == 'n':
                visited[v.id][direction] = v.n_to.id
            elif direction == 's':
                visited[v.id][direction] = v.s_to.id
            elif direction == 'e':
                visited[v.id][direction] = v.e_to.id
            else:
                visited[v.id][direction] = v.w_to.id
